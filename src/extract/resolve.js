...
// definition in the same file first, then anywhere in the same repo. We never
// resolve a call across repos by name — C and TS don't share a namespace, so a
// shared name like `start` would be a false edge. Genuine cross-repo links flow
// through Contract nodes instead (see contracts.js).
//
...
// files we didn't re-parse. Each must look like {id, repo, file, name, kind}.
export function resolveCalls(graph, calls, log = () => {}, extraDefs = null) {
  // Index definitions by name -> [{id, repo, file}], excluding module symbols.
  ...
  for (const c of calls) {
    ...
    const sameFile = cands.filter((s) => s.repo === c.repo && s.file === c.relPath);
    const sameRepo = cands.filter((s) => s.repo === c.repo);
    const scope = sameFile.length ? sameFile : sameRepo;

    if (scope.length === 0) {
      unresolved++; // only cross-repo name matches existed -> not a real call
      continue;
    }
