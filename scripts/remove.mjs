#!/usr/bin/env node
// Hard uninstall of codegraph from a project. Deletes EVERYTHING codegraph put in
// or around the project, and nothing else:
//   1. the managed directive block in <project>/CLAUDE.md (between the sentinels)
//   2. the codegraph entry in <project>/.gitignore (+ its comment)
//   3. the <project>/.codegraph/ folder (state, log, and the graph.db itself)
//   4. a dangling ~/.codegraph symlink left pointing at a deleted .codegraph
//
//   node remove.mjs <project> [--dry-run]
//
// The graph is a single SQLite file inside .codegraph/, so deleting that folder
...
import { codegraphDir, GITIGNORE_LINE } from './lib/state.mjs';
...
// Strip the codegraph entry (and the contiguous codegraph comment lines above it)
...
function stripGitignore(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (t === GITIGNORE_LINE || t === '/.codegraph/') {
      // drop any contiguous codegraph comment lines we just emitted
      while (out.length && out[out.length - 1].trim().startsWith('#') &&
             /codegraph/i.test(out[out.length - 1])) out.pop();
...
async function main() {
  ...
  const cgDir = codegraphDir(project);
  const did = (m) => log(`${o.dryRun ? '  • would ' : '  ✓ '}${m}`);

  log(`codegraph removal for ${project}${o.dryRun ? '  (DRY RUN — no changes)' : ''}`);
  ...
  if (existsSync(gi)) {
    ...
    if (next !== cur) { if (!o.dryRun) writeFileSync(gi, next.replace(/\n*$/, '\n')); did('remove the .codegraph/ entry from .gitignore'); }
    else log('  – no codegraph entry in .gitignore (skipped)');
  }

  // 3. the .codegraph/ folder (contains graph.db + state) --------------------
  if (existsSync(cgDir)) { if (!o.dryRun) rmSync(cgDir, { recursive: true, force: true }); did(`delete ${cgDir} (graph.db + state)`); }
  else log('  – no .codegraph/ folder (skipped)');

  // 4. dangling ~/.codegraph symlink (legacy runtime pointer) ----------------
  const home = join(process.env.HOME || '', '.codegraph');
  try {
    if (lstatSync(home).isSymbolicLink()) {
      ...
      if (!existsSync(home) || target === cgDir) { // dangling, or pointed at what we deleted
        ...
        did(`remove the dangling ~/.codegraph symlink (was -> ${target})`);
      }
    }
  } catch { /* no ~/.codegraph */ }

  log(o.dryRun ? 'Dry run complete — nothing was changed.' : 'codegraph removed. Your source, the rest of CLAUDE.md, and the rest of .gitignore are untouched.');
}
